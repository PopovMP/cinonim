...
function compileArraySet(arraySet, output, depth)
{
	...
	const pointerExpr    = getPointer(arraySet, depth)
	const elemValueExpr  = compileExpression(arraySet.nodes[1].nodes, 0, depth)

	add(`(${dataType}.store ${pointerExpr} ${elemValueExpr})`, output, depth)
}
...
function getPointer(arrayNode, depth)
{
	...
	const arrayIndexExpr = compileExpression(arrayNode.nodes[0].nodes, 0, depth)
	const shiftVal       = arrMeta.elemSize === 4 ? 2 : 3

	return `(i32.add (i32.const ${arrMeta.pointer}) (i32.shl ${arrayIndexExpr} (i32.const ${shiftVal})))`
}
