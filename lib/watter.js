...
function astToWat(moduleNode)
{
	...
	if (arrayAllocationTable.length > 0) {
		const allocatedSize = arrayAllocationTable.reduce((size, arr) => arr.arrLength * arr.elemSize, 0)
...
function allocateArray(node)
{
	...
	const pointer = arrayAllocationTable.reduce((size, arr) => arr.arrLength * arr.elemSize, 0)
