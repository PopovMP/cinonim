...
/**
 * @typedef {Object} ArrayMeta
 *
 * @property {string} arrName
 * @property {number} arrId
 * @property {number} pointer   - i32
 * @property {number} elemSize  - 4 or 8
 * @property {number} arrLength - i32
 * @property {number} isBusy    - 1 - busy, 0 - free
 */
...
function astToWat(moduleNode)
{
	...
	// Reserve memory
	if (arrayAllocationTable.length > 0) {
		const allocatedSize = arrayAllocationTable.reduce((size, meta) => size + meta.arrLength * meta.elemSize, 0)
		const pages = Math.ceil(allocatedSize / (64*1024))

		// Hack to put memory allocation after a possible import
		for (let i=0; i<output.length; i++) {
			if (output[i].startsWith('    (func')) {
				output[i] = `    (memory ${pages})\n` + output[i]
				break;
			}
		}
	}
...
function compileAst(node, output, depth)
{
	...
	// Array allocation
	// int foo[42];
	if (node.type === NodeType.arrayAlloc) {
		allocateArray(node)
		return
	}
...
function compileForm(node, output, depth)
{
	...
	// Array set
	// foo[i] = ...;
	if (node.type === NodeType.arraySet) {
		compileArraySet(node, output, depth)
		return
	}
...
function compileExpression(nodes, index, depth)
{
	...
	switch (node.type) {
		...
		case NodeType.arrayGet:
			return `(${node.dataType}.load ${getPointer(node, depth)})`
		case NodeType.cast:
 ...
 * Allocates an array
 */
function allocateArray(node)
{
	const arrName   = node.value
	const dataType  = node.dataType
	const elemSize  = dataType === DataType.i32 || dataType === DataType.f32 ? 4 : 8
	const arrLength = node.data[0]

	if (Object.hasOwn(arraysNameIndex, arrName))
		die(`Array ${arrName} is already declared`, node)

	const pointer = arrayAllocationTable.reduce((size, meta) => size + meta.arrLength * meta.elemSize, 0)
	const arrId   = Object.keys(arraysNameIndex).length
	arraysNameIndex[arrName] = arrId

	/** @type{ArrayMeta} */
	const arrayMeta = {arrName, arrId, arrLength, elemSize, pointer, isBusy: 1}

	arrayAllocationTable.push(arrayMeta)
}

/**
 * Compiles array set value
 * foo[i] = ...;
 *
 * @param {Node}     arraySet
 * @param {string[]} output
 ...
 * @return {void}
 */
function compileArraySet(arraySet, output, depth)
{
	const dataType   = arraySet.dataType
	const pointerWat = getPointer(arraySet, depth)
	const valueWat   = compileExpression(arraySet.nodes[1].nodes, 0, depth)

	add(`(${dataType}.store ${pointerWat} ${valueWat})`, output, depth)
}

function getPointer(arrayNode, depth)
{
	const arrName  = arrayNode.value
	const arrId    = arraysNameIndex[arrName]
	const arrMeta  = arrayAllocationTable[arrId]
	const indexWat = compileExpression(arrayNode.nodes[0].nodes, 0, depth)
	const shiftVal = arrMeta.elemSize === 4 ? 2 : 3

	return `(i32.add (i32.const ${arrMeta.pointer}) (i32.shl ${indexWat} (i32.const ${shiftVal})))`
}
