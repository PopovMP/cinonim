...
function parseStatement(parentNode, tokens, index)
{
	...
	if (isWord(t0) && isPunctuation(t1, '[')) {
		...
		const arrayIndex  = makeNode(arraySet, NodeType.expression, arrName, DataType.i32, t2)
		index = parseExpression(arrayIndex, tokens, index+2)
		const arrayValue  = makeNode(arraySet, NodeType.expression, arrName, arrDataType, tokens[index+1])
...
function parseExpressionChain(parentNode, tokens, index)
{
	...
	if (isWord(t0) && isPunctuation(t1, '[')) {
		...
		const arrayIndex  = makeNode(arrayGet, NodeType.expression, arrName, DataType.i32, t2)
		index = parseExpression(arrayIndex, tokens, index+2)
		return parseExpressionChain(parentNode, tokens, index)
	}
